...
parser.add_argument('--class_num', type=int, help = 'The number of classes.', default = 0)
parser.add_argument('--model_load', type=str, help = 'model Name of Timm library or your own model path', default='inception_v4')
...
modelfile_save_name = args.model_save_name
...
try :
    ...
    model = timm.create_model(args.model_load, pretrained=True)
    num_features = model.last_linear.in_features
#    전이 학습(transfer learning): 모델의 출력 뉴런 수를 2개로 교체하여 마지막 레이어 다시 학습
    model.fc = nn.Linear(num_features, args.class_num)
    
print(model)

if args.gpus != 0 :
    model = torch.nn.DataParallel(model, device_ids=gpu_list) # GPU병렬연결
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if acc_past < epoch_acc :
            torch.save(model,'{}.pth'.format(modelfile_save_name))
            acc_past = epoch_acc